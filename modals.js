...
`.trim();
            ...
            if (addressLine && reportType && deadline) {
                ...
                emailPayloads.push({
                    ...
                    target_area: calcArea,
                    estimated_cost: baseCost > 0 ? baseCost : null,
                    // These are the extra fields just for the email!
                    travel_hrs: trvH,
                    insp_hrs: inspH,
                    rep_hrs: repH,
                    discount_applied: parseFloat(row.dataset.discountApplied || '0')
                });
        ...
        const { error: emailError } = await supabase.functions.invoke('send-request-email', {
            body: { requests: emailPayloads } // <-- Change 'inserts' to 'emailPayloads'
        });
...
window.openEditReportModal = (id) => {
    ...
    // ADD THIS LINE:
    if (document.getElementById('editRepName')) document.getElementById('editRepName').value = report.name || '';
    ...
    setTimeout(() => document.getElementById('editReportBody').focus(), 100);
};

// --- CATCH PASTED IMAGES IN EDIT REPORT MODAL ---
document.getElementById('editReportBody')?.addEventListener('paste', (e) => {
    const items = e.clipboardData?.items;
    if (!items) return;

    const input = document.getElementById('editRepImages');
    const dt = new DataTransfer();
    let hasImage = false;

    // Keep any previously selected or pasted files in the input
    if (input.files) {
    ...
    // Loop through pasted items and extract images
    for (let item of items) {
        if (item.type.indexOf('image') !== -1) {
            const file = item.getAsFile();
            // Give the pasted file a unique name so Supabase accepts it properly
            const ext = file.name && file.name.includes('.') ? file.name.split('.').pop() : 'png';
            const renamedFile = new File([file], `pasted_img_${Date.now()}.${ext}`, { type: file.type });
    ...
    if (hasImage) {
        // Apply the combined files back to the input and update the UI label
        input.files = dt.files;
        window.updateFileName(input, 'editRepImagesName');
    }
});
...
document.getElementById('editReportForm')?.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const updateData = {
            name: document.getElementById('editRepName') ? document.getElementById('editRepName').value.trim() : null,
